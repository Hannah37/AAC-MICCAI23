...
def print_iou(iu, mean_dice, mean_pixel_acc, class_names=None, show_no_back=False, no_print=False):
    n = iu.size
    lines = []
    for i in range(n):
    ...
    if show_no_back:
        lines.append('----------------------------     %-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%' % ('mean_IU', mean_IU * 100, 'mean_DICE', mean_DICE * 100, 'mean_IU_no_back', mean_IU_no_back*100,
