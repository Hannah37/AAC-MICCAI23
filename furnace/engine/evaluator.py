...
class Evaluator(object):
    ...
    def epoch_run(self, model_path, model_indice, select_classes):
        # print('model_path: ', model_path)
        # print('os.path.join(model_path, epoch-%s.pth % model_indice: ', os.path.join(model_path, 'epoch-%s.pth' % model_indice))
        if os.path.exists(model_path) and os.path.exists(os.path.join(model_path, 'epoch-%s.pth' % model_indice)):
            model = os.path.join(model_path, 'epoch-%s.pth' % model_indice)
        else:
            print("No model exist for evaluation !!")
        ...
        result_line, miou, mdice = self.single_process_evalutation(select_classes)
        # miou = result_line.split('mean_IU')[1][2:8]
        return miou, mdice
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        if '.pth' in model_indice:
            ...
            sorted_models = [x for _, x in sorted(zip(model_idx, sorted_models))]
            model_idx = np.array(sorted([int(i) for i in model_idx]))
        ...
        for model in models:
            ...
            if len(self.devices) == 1:
                result_line, _, _ = self.single_process_evalutation()
            # else:
            #     result_line = self.multi_process_evaluation()        
            ...
            results.write('\n')
            result_list.append(result_line.split('mean_IU')[1][2:8])
            results.flush()
    ...
    def single_process_evalutation(self, select_classes):
        ...
        for idx in tqdm(range(self.ndata)):
            dd = self.dataset[idx]
            results_dict = self.func_per_iteration(dd, select_classes, self.devices[0])
            all_results.append(results_dict)
        result_line, miou, mdice = self.compute_metric(all_results)
        logger.info(
            'Evaluation Elapsed Time: %.2fs' % (
                    time.perf_counter() - start_eval_time))
        return result_line, miou, mdice
    ...
    def multi_process_evaluation(self):
        ...
        for d in range(nr_devices):
            ...
            logger.info(
                'GPU %s handles %d data.' % (device, len(shred_list)))
        ...
        result_line, _, _ = self.compute_metric(all_results)
        logger.info(
            'Evaluation Elapsed Time: %.2fs' % (
                    time.perf_counter() - start_eval_time))
        return result_line
    ...
    def whole_eval(self, img, image_width, image_height, select_classes, device=None):     

        if torch.is_tensor(img): # no pretrained weights
            x_tensor = img.cuda(device).unsqueeze(0)

        else: # imagenet pretrained
            x_tensor = torch.from_numpy(img).cuda(device).unsqueeze(0)

        self.val_func = self.val_func.cuda(device)
        # print('x_tensor: ', x_tensor.shape) # [1, 3, 512, 608]
        self.val_func.eval()
        pred = self.val_func(x_tensor)
        
        pred_argmax = pred.argmax(1) # NCHW
        out = torch.zeros_like(pred).scatter_(1, pred_argmax.unsqueeze(1), 1.)
    ...
    def scale_process(self, img, ori_shape, image_width, image_height, stride_rate,
                      device=None):
